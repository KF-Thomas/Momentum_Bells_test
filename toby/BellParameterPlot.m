...
slist = 0.*phi;
%% Parameters:
cycles = 1E2; % C = number of test cycles simulated each time.
...
title("Bell Parameter vs Phase,$\lambda = 0.1$, $R = 8$\%, $D = 0$, B = 0.05, $C = 10^2$","Interpreter","latex")

%% Write to File:
A = [phi; slist];
data = fopen('R8D00B005C1e2L01.txt','w');
fprintf(data,'%6s %12s\n','\phi','E(\phi)');
