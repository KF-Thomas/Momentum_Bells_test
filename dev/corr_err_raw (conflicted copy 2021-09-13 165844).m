function raw_unc = corr_err_raw(corr_func,corr_opts,counts)
warning('off','all')
...
est_fun = @(x) corr_wrap(corr_func,counts,corr_opts,x);
m=(1:length(counts))';
% ci = bootci(corr_opts.num_samp_rep,{est_fun,m},'Alpha',1-0.6826895);
ci = bootci(corr_opts.num_samp_rep,{est_fun,m},'UseParallel',true);
...
raw_unc.range = range(ci)./2;%boot_out.results.se_fun_whole_unweighted(1,1:grid_size);
raw_unc.ci = ci;
...
function out = corr_wrap(corr_func,counts,corr_opts,x)
corr_opts.attenuate_counts = 1.0;
corr_opts.plots = false;
corr_opts.print_update = false;
rawcorr = corr_func(corr_opts,counts(:,x));
