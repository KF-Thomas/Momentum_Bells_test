...
data_folder='20211206_scaning_across_freq\bs_high_100_kHz_evap_0_844_MHz';%'20211206_scaning_across_freq\double_mirror_120_kHz_and_80_kHz';%

data_folder_norm='20211206_scaning_across_freq\norm_0_844_MHz';
%double halo
...
cal_dens_top = out_norm.v_top_dens;
v_btm_dens = out.v_btm_dens;
cal_dens_btm = out_norm.v_btm_dens;
...
%%
    ...
    clf
    set(gcf,'position',[516    42  689   774])%[916    42   389   774]
    h(3)=subplot(1,2,2);
plot(a,(x+rad_shift).*1e3,'b','LineWidth',1.5)
ylim([-0.13,0.12].*1e3)
xlim([0 1])
set(gca,'FontSize',17)
set(gca,'yticklabel',[])
xlabel('Proportion')
legend('$C_{-1}$')
set(h(3), 'position', [0.62, 0.1, 0.3, 0.6] );
% h(1)=subplot(2,2,1);
...
set(h(2), 'position', [0.1, 0.1, 0.5, 0.6] );
box on
xlim([-73 73])
set(gca,'FontSize',17)
